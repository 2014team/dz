...
public class MonochromeLayer extends Layer {
	  ...
	  public MonochromeLayer(PApplet pApplet,Frame frame, PImage img, WeaveConfig config) throws Exception {
	    super(frame, img);
	    ...
	    this.working_img.filter(pApplet.GRAY);
	  ...
	  public boolean drawNextLine() {
	    ...
	    for (int i = 0; i < line_len; ++i) {
	      float c = pApplet.red(this.working_img.get((int)(point.x), (int)(point.y)));

	      c += this.config.remove_intensity;
	      if (c > 255) c = 255;

	      this.working_img.set((int)(point.x), (int)(point.y), pApplet.color(c));
	      point.add(step);
	    }
